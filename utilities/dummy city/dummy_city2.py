    ...
    def get_amenities(self):
        ...
        for i in range(self.size):
            for j in range(self.size):
                if self.grid[i, j] == 2:  # Houses
                    amenities.append(('house', f'House {len(amenities) + 1}', self.get_coordinates(i, j)))
                elif self.grid[i, j] == 3:  # Hospitals
                    amenities.append(('hospital', 'Hospital', self.get_coordinates(i, j)))
                elif self.grid[i, j] == 4:  # Schools
                    amenities.append(('school', 'School', self.get_coordinates(i, j)))
                elif self.grid[i, j] == 5:  # Parks
                    amenities.append(('park', 'Park', self.get_coordinates(i, j)))
                elif self.grid[i, j] == 6:  # Shops
                    amenities.append(('shop', f'Shop {len(amenities) + 1}', self.get_coordinates(i, j)))
                elif self.grid[i, j] == 7:  # Offices
                    amenities.append(('office', f'Office {len(amenities) + 1}', self.get_coordinates(i, j)))

        return amenities

    def get_coordinates(self, x, y):
        # Converts grid coordinates to latitude and longitude (example conversion)
        lat = 25 + (x / self.size)  # Example latitude adjustment
        lon = 82 + (y / self.size)  # Example longitude adjustment
        return lat, lon

    def share_map(self, filename='amenities.csv'):
        amenities = self.get_amenities()
        data = []
        for amenity in amenities:
            amenity_type, name, (lat, lon) = amenity
            data.append([amenity_type, name, lat, lon])

        df = pd.DataFrame(data, columns=["Amenity Type", "Name", "Latitude", "Longitude"])
